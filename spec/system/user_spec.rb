...
RSpec.describe 'User' do
  ...
  context 'when user edit profile' do
    before { sign_in_user(user) }

    it 'when user edit profile successfully' do
      edit_profile_success(user)

      expect(page).to have_text('Your account has been updated successfully.')
    end

    it 'when user failed to edit profile' do
      edit_profile_failed(user)
